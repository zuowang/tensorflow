...
class SeluTest(tf.test.TestCase):

    def _npSelu(self, np_features):
        return np.where(np_features < 0, np.exp(np_features) - 1, np_features / (1 + np.exp(-np_features)))

    def testNpSelu(self):
        self.assertAllClose(
            np.array([[-0.59343034025, 0.7, -0.39346934028, 0.3, -0.09516258196],
                      [0.1, -0.25918177931, 0.5, -0.5034146962, 0.9]]),
            self._npSelu(np.array([[-0.9, 0.7, -0.5, 0.3, -0.1], [0.1, -0.3, 0.5, -
            0.7, 0.9]])))

    def _testSelu(self, np_features, use_gpu=False):
        np_selu = self._npSelu(np_features)
        with self.test_session(use_gpu=use_gpu):
            selu = tf.nn.selu(np_features)
            tf_selu = selu.eval()
